...
void myDisplay(void)
{
	...
	while (currentGroup)
	{
		...
		GLfloat specularExp,diffuseL[3],ambientL[3],specularL[3];

		specularExp = currentMaterial.shininess;
		...
		for (i = 0; i < currentGroup->numtriangles; i++)
		{
			...
			for (temp = 0; temp < 3; temp++)
				colourRGB[temp] = ambientL[temp] + diffuseL[temp] * dotPnl +specularL[temp] * pow(dotPnh, specularExp);
			...
			if (gcount%2==1)
			{
				...
				for (temp = 0; temp < 3; temp++)
					colourRGB1[temp] = ambientL[temp] + diffuseL[temp] * dotPnl1 + specularL[temp] * pow(dotPnh1, specularExp);
				...
				for (temp = 0; temp < 3; temp++)
					colourRGB2[temp] = ambientL[temp] + diffuseL[temp] * dotPnl2 + specularL[temp] * pow(dotPnh2, specularExp);
				...
				for (temp = 0; temp < 3; temp++)
					colourRGB3[temp] = ambientL[temp] + diffuseL[temp] * dotPnl3 + specularL[temp] * pow(dotPnh3, specularExp);
			}
